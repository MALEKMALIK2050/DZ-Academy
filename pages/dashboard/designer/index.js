...
export default function DesignerDashboard() {
  ...
  const handleSendMessage = async () => {
    ...
    if (!newMsg.receiverId || !newMsg.content) {
      return setMsgError("المستلِم والرسالة إلزاميان");
    }
    ...
    try {
      ...
      if (res.ok) {
        setMsgSuccess("✅ تم إرسال الرسالة!");
        setNewMsg({ ...newMsg, content: "" });
        setMessages((prev) => [...prev, data]);
      } else {
        setMsgError(data.error || "خطأ في الإرسال");
      }
    } catch {
      setMsgError("خطأ في الخادم");
    } finally {
      setSendingMsg(false);
    }
  };
  ...
  // Socket للشارات في الوقت الفعلي
  ...
  // تمييز الرسائل كمقروءة عند فتح تبويب الدردشة
  useEffect(() => {
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
            <div>
              <h1 style={{ 
                ...
                🎨 مرحبًا <span style={{ color: "#8b5cf6" }}>{user?.prenom}</span>!
              </h1>
              <p style={{ color: "#718096" }}>أدر دروسك ومحتوياتك التعليمية</p>
            </div>
          ...
          {tab === "overview" && (
            <div className="stats-grid">
              <div style={{ background: "linear-gradient(135deg,#8b5cf6,#7c3aed)", color: "white", padding: "1.5rem", borderRadius: "14px", textAlign: "center", boxShadow: "0 4px 15px rgba(0,0,0,0.1)" }}>
                ...
                <div style={{ fontSize: "0.85rem", opacity: 0.9 }}>إجمالي الدروس</div>
              </div>
              <div style={{ background: "linear-gradient(135deg,#06b6d4,#0891b2)", color: "white", padding: "1.5rem", borderRadius: "14px", textAlign: "center", boxShadow: "0 4px 15px rgba(0,0,0,0.1)" }}>
                ...
                <div style={{ fontSize: "0.85rem", opacity: 0.9 }}>منشورة</div>
              </div>
              <div style={{ background: "linear-gradient(135deg,#f59e0b,#d97706)", color: "white", padding: "1.5rem", borderRadius: "14px", textAlign: "center", boxShadow: "0 4px 15px rgba(0,0,0,0.1)" }}>
                ...
                <div style={{ fontSize: "0.85rem", opacity: 0.9 }}>مسودات</div>
              </div>
          ...
          {tab === "courses" && (
            <div>
              <div style={{ display: "flex", gap: "1rem", marginBottom: "2rem" }}>
                <h2>📚 دروسي المُنشأة ({courses.length})</h2>
              </div>

              {courses.length === 0 ? (
                <p style={{ color: "#475569" }}>لا يوجد أي درس — ابدأ بإنشاء درسك الأول!</p>
              ) : (
                <>
          ...
          {tab === "messages" && (
            <div>
              <div style={{ background: "#f8fafc", padding: "1.25rem", borderRadius: "12px", marginBottom: "1.5rem", border: "1px solid #e2e8f0" }}>
                <h3 style={{ margin: "0 0 1rem" }}>✉️ رسالة جديدة</h3>
                ...
                <select
                  ...
                  style={{ width: "100%", padding: "0.6rem", border: "1px solid #cbd5e0", borderRadius: "6px", marginBottom: "0.75rem", direction: "rtl" }}
                >
                ...
                <textarea
                  ...
                  style={{ width: "100%", padding: "0.6rem", border: "1px solid #cbd5e0", borderRadius: "6px", height: "80px", resize: "vertical", direction: "rtl" }}
                />
              ...
              {Object.keys(conversations).length === 0 ? (
                <p style={{ color: "#475569" }}>لا توجد محادثات.</p>
              ) : (
                Object.entries(conversations).map(([otherId, conv]) => (
                  <div key={otherId} style={{ background: "#f8fafc", borderRadius: "12px", marginBottom: "1rem", overflow: "hidden", border: "1px solid #e2e8f0" }}>
                    <div style={{ background: "#f1f5f9", padding: "0.75rem 1rem", fontWeight: "bold", display: "flex", justifyContent: "space-between" }}>
                      <span>{conv.user?.prenom} {conv.user?.nom} <span style={{ fontSize: "0.8rem", color: "#64748b", marginRight: "0.5rem" }}>({conv.user?.role})</span></span>
                      {conv.messages.filter(m => m.receiverId === user?.id && !m.lu).length > 0 && (
                        <span style={{ background: "#dc2626", color: "white", padding: "0.1rem 0.5rem", borderRadius: "10px", fontSize: "0.8rem" }}>
                          {conv.messages.filter(m => m.receiverId === user?.id && !m.lu).length} غير مقروء
                        </span>
                      )}
                    </div>
                    <div style={{ padding: "1rem", maxHeight: "250px", overflowY: "auto" }}>
                      {conv.messages.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).map((m) => (
                        <div key={m.id} style={{ display: "flex", justifyContent: m.senderId === user?.id ? "flex-start" : "flex-end", marginBottom: "0.5rem" }}>
                          <div style={{ background: m.senderId === user?.id ? "#8b5cf6" : "#e2e8f0", color: m.senderId === user?.id ? "white" : "#1e293b", padding: "0.5rem 0.8rem", borderRadius: "12px", maxWidth: "70%", fontSize: "0.9rem" }}>
                            ...
                            <div style={{ fontSize: "0.7rem", opacity: 0.7, marginTop: "0.2rem" }}>
                              ...
                              {m.receiverId === user?.id && !m.lu && (
                                <button onClick={() => handleMarkRead(m.id)} style={{ marginRight: "0.5rem", background: "none", border: "none", color: "inherit", cursor: "pointer", fontSize: "0.7rem" }}>
                                  ✓ تمييز كمقروء
                                </button>
                              )}
                            </div>
