...
import { NiveauScolaire, StatutProfilCompletion, Role } from "@prisma/client";
...
export async function updateProfileCompletion(userId: number): Promise<void> {
  ...
  let status: StatutProfilCompletion;
  if (percentage === 0) {
    status = StatutProfilCompletion.INCOMPLET;
  } else if (percentage < 100) {
    status = StatutProfilCompletion.PARTIELLEMENT_COMPLET;
  } else {
    status = StatutProfilCompletion.COMPLET;
  }
 ...
 */
export const statutProfilLabels: Record<StatutProfilCompletion, string> = {
  [StatutProfilCompletion.INCOMPLET]: "Incomplet",
  [StatutProfilCompletion.PARTIELLEMENT_COMPLET]: "Partiellement complété",
  [StatutProfilCompletion.COMPLET]: "Complété"
};
